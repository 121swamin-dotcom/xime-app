...
import { BarChart2, Calendar, Users, ChevronRight, AlertCircle, Clock } from 'lucide-react';
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <div className="mb-6">
        ...
        <p className="text-slate-400 text-sm mt-0.5">{user?.roll_number} · {user?.section}</p>
      </div>
      ...
      {target && (
        <div className="card p-5 mb-5">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">Role Targets</p>
          <div className="space-y-2">
            <button onClick={() => navigate(`/roles/${target.primary_category}`)}
              className="flex items-center justify-between w-full group">
              <div className="text-left">
                <span className="text-xs font-semibold text-[#CC0000]">PRIMARY</span>
                <p className="font-semibold text-slate-800 text-sm mt-0.5">{target.primary_description}</p>
              </div>
              <ChevronRight size={14} className="text-slate-300 group-hover:text-[#CC0000] transition-colors" />
            </button>
            {target.secondary_category && (
              <button onClick={() => navigate(`/roles/${target.secondary_category}`)}
                ...
                <div className="text-left">
                  <span className="text-xs font-semibold text-slate-400">SECONDARY</span>
                  <p className="font-medium text-slate-600 text-sm mt-0.5">{target.secondary_description}</p>
                </div>
                <ChevronRight size={14} className="text-slate-300 group-hover:text-[#CC0000] transition-colors" />
              </button>
            )}
          ...
          <button onClick={() => navigate('/electives')}
            className="mt-3 text-xs text-[#CC0000] hover:underline">Change targets</button>
        </div>
      )}
      ...
      {target && (
        <div className="mb-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            <ScoreCard label="Overall Readiness" pct={readiness.overall}    size="large" />
            <ScoreCard label="Competencies"       pct={readiness.competency} />
            <ScoreCard label="TTFs"               pct={readiness.ttf} />
          </div>
          <p className="text-xs text-slate-400 text-center mt-2">Overall = Competency 60% + TTF 40%</p>
          <div className="text-center mt-1">
            <button onClick={() => navigate('/competencies')}
              className="text-xs text-[#CC0000] hover:underline">Update ratings →</button>
          </div>
        </div>
      )}
      ...
      <div className="card p-5 mb-5">
        <div className="flex items-center justify-between mb-4">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">My Electives</p>
          <button onClick={() => navigate('/electives')}
                    ...
                    {byTerm[term].length === 0
                      ...
                      : byTerm[term].map((e) => (
                          <div key={e.course_code}
                            className="text-xs bg-slate-50 border border-slate-100 rounded-md px-2.5 py-1.5 text-slate-600">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard icon={<BarChart2 size={18} />} label="Evidence Submitted"
          ...
          sub={`${evidence.approved} approved`} color="red"
...
function LoadingState() {
  return (
    <div className="p-6 max-w-5xl mx-auto animate-pulse">
      ...
      <div className="card p-5 h-28 mb-5" />
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[1,2,3].map(i => <div key={i} className="card p-4 h-24" />)}
      </div>
